...
#define ES_TARGET_VERSION "v3.1.0"
...
int easy_setup_start(const char* ifname, int debugflag) {
    int i;
    int ret = 0;

    debug_enable = debugflag;
    strncpy(g_wlanname, ifname, 16 - 1);
...
int easy_setup_query(int timeout) {
    int ret = 0;
    tlv_t* query = NULL;
    ...
    int tries = (1000*timeout)/QUERY_INTERVAL_MS;
    while (tries--) {
...
int easy_setup_ioctl(int cmd, int set, void* param, int size) {
    ...
    strncpy(ifr.ifr_name, g_wlanname, IFNAMSIZ);
